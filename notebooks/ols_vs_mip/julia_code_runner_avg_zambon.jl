    ...
    measurements = 300
    observations = 300
    observations_test = 300
    noise_snr=  [1,1]
    base = 1
    coeff = 1
    lambda = 100#base * 10.0^coeff
    ...
    #     # 
        ...
        # Set BigM values
        BigM =ones(size(beta_matrix))  .* 10000   # or use   beta_matrix_max_values
        BigM_ =  ones(size(beta_matrix))  .* -10000  # or use    beta_matrix_min_values
...
# end
    ...
    for (key, value) in cumulative_metrics_estimate
        average_metrics_estimate[key] = float(value) / N_simulations
    end

    for (key, value) in cumulative_metrics_ols
        average_metrics_ols[key] = float(value) / N_simulations
    end

    for (key, value) in cumulative_metrics_real
        average_metrics_real[key] = float(value) / N_simulations
    end
    ...
    println("\n\n\n\n")
    println("Average ISE coeff for the estimated model: ", average_metrics_estimate["ISE_Coefficients"])
    println("Average MSE for the estimated model: ", average_metrics_estimate["MSE_Predictions"])
    println("Average RMSE for the estimated model: ", average_metrics_estimate["RMSE_Predictions"])
    println("Average RMSE coeff for the estimated model: ", average_metrics_estimate["RMSE_Coefficients"])