...
function mip_functional_regression(Y, Z, BigM,BigM_; intercept = false, group_limit=Inf, initial_beta = nothing, initial_group = nothing, initial_alpha = nothing)
    ...
    Z = reshape(Z_permuted, _obs, _predictors * _basi)
    ...
    "NonConvex" => 1,  # Allow for non-convex optimization
    ...
    @variable(model, beta[1:p*r])
    @variable(model, group[1:p], Bin)
    alpha = intercept ? @variable(model) : 0
    
    @objective(model, Min, mean(sum((Y[i] - alpha - sum(Z[i,:]' * beta[:]))^2 for i in 1:n)))
    ...
    for j in 1:p
        for k in 1:r
            @constraint(model, beta[(j-1)*r + k] <= group[j] * BigM[j])
            @constraint(model, beta[(j-1)*r + k] >= group[j] * BigM_[j])
        end
    end
    ...
    beta_star =  permutedims(reshape(beta_star,_basi,_predictors), [2, 1])
