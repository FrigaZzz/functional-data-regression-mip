...
function plot_combined_predicted_curve(γ_matrix, γ_star, basis_values,tb, folder_path, show_plot =false )
    ...
    for j in 1:size(γ_matrix, 1)  # Loop over all curves
        ...
        p = plot(combined_curve_matrix, label="γ_matrix", color=:blue)
        plot!(combined_curve_star, label="γ_predicted", color=:red, linestyle=:dash)
...
function plot_combined_predicted_curve_paper(γ_matrix, γ_star, basis_values, predictors,  time_domains, folder_path, show_plot=false)
    mkpath(folder_path)  # Ensure the plots directory exists
    rm(folder_path, recursive=true)  # Clear the directory
    mkpath(folder_path)  # Recreate the directory
