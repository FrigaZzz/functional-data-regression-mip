...
function mip_functional_regression(Y, Z, weights, lambda, lambda_group, BIG_M, group_limit=Inf)
    n, p, r = size(Z)
    ...
    @objective(model, Min, sum(weights[i] * (Y[i] - sum(Z[i, j, k] * beta[j, k] for j in 1:p, k in 1:r) )^2 for i in 1:n) +
