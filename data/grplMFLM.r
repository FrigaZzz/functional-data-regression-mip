...
grplMFLM = function(Y, X, Tps, lambda, phi, dfs = 10,
adapt1 = NULL, adapt2 = NULL, ...){
  ...
  if (length(dfs) != nfunc)
	...
	grpl = grplasso(x = Z, y = Y, index = index, model = LinReg(), lambda = lambda,
	                control = grpl.control(max.iter = 5000), standardize = F, ...)
...
cv.grplMFLM <- function(k, Y, X, Tps, lambda, phi, dfs = 10, ...)
  {
    ...
    for (wk in 1:k)
      {
       ...
       # phi walues
        for (wp in 1:length(phi))
          {
            # model fitting
            grplk <- grplMFLM(Y = Yk, X = Xk, Tps = Tps, lambda = lambda, 
            phi = phi[wp], dfs = dfs, ...)
...
grplLogit = function(Y, X, Tps, lambda, phi, dfs = 10,
adapt1 = NULL, adapt2 = NULL, ...){
...
cv.grplLogit <- function(k, Error = "logScore", Y, X, Tps, lambda, phi,
dfs = 10, ...)
  {
  # K-fold cross-validation function
    n <- length(Y)
    ...
    for (wk in 1:k)
      {
       ...
       # phi values
        for (wp in 1:length(phi))
          {
            # model fitting
            grplk <- grplLogit(Y = Yk, X = Xk, Tps = Tps, lambda = lambda,
            phi = phi[wp], dfs = dfs, ...)
